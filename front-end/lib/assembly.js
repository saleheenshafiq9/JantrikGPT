...
const API_TOKEN = "5ed5bb07429548bca9003c06807223f3"

// Function to upload a local file to the AssemblyAI API
async function upload_file(api_token, blob) {
...
export default async function getTextFromAssemblyAI(blob) {
  // -----------------------------------------------------------------------------
  ...
  const uploadUrl = await upload_file(API_TOKEN, blob)
  ...
  // const uploadUrl = URL.createObjectURL(blob)
